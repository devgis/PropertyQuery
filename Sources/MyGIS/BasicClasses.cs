...
namespace MyGIS
{
    ...
    public class CS_WGS84
    {
        public static double SemiMajorAxis = 6378137;
        public static double SemiMinorAxis = 6356752.314245;
    }

    public class UTM_Vertex
    {
        public GISVertex v;
        public int ZoneNumber;
        public bool NorthOrSouth; //北半球: true, 南半球: false

        public UTM_Vertex(double _X, double _Y, int _ZoneNumber, bool _NorthOrSouth)
        {
            v = new GISVertex(_X, _Y);
            ZoneNumber = _ZoneNumber;
            NorthOrSouth = _NorthOrSouth;
        }
    }
    ...
    public class CS_UTM
    {
        ...
        public static double SemiMajorAxis = CS_WGS84.SemiMajorAxis;
        public static double SemiMinorAxis = CS_WGS84.SemiMinorAxis;
        public static double FalseEasting = 500000;
        public static double ScaleFactor = 0.9996;
        static double EquatorLength = 40000000;
        static double SouthOffset = 10000000;

        static double p1 = Math.Pow((Math.Pow(SemiMajorAxis, 2) - Math.Pow(SemiMinorAxis, 2)), 0.5) / SemiMinorAxis;
        static double p2 = Math.Pow(p1, 2);
        static double p3 = p2 * 3 / 4;
        static double p4 = Math.Pow(p3, 2) * 5 / 3;
        static double p5 = Math.Pow(p3, 3) * 35 / 27;
        static double p6 = Math.Pow(SemiMajorAxis, 2) / SemiMinorAxis;
        static double p7 = EquatorLength / (2 * Math.PI);
        static double p8 = (p7 * ScaleFactor);

        static double Rad2Deg = 180 / Math.PI;

        public static GISVertex UTM2WGS(GISVertex v, int ZoneNumber, bool NorthOrSouth)
        {
            double easting = v.x - FalseEasting;
            double northing = NorthOrSouth ? v.y : v.y - SouthOffset;
            double centralMeridian = ((ZoneNumber * 6.0) - 183.0);

            double a1 = northing / p8;
            double a2 = (p6 / Math.Pow(1 + (p2 * Math.Pow(Math.Cos(a1), 2)), 0.5)) * ScaleFactor;
            double a3 = easting / a2;
            double a4 = Math.Sin(2 * a1);
            double a5 = a4 * Math.Pow((Math.Cos(a1)), 2);
            double a6 = a1 + a4 / 2;
            double a7 = (a6 * 3 + a5) / 4;
            double a8 = (5 * a7 + a5 * Math.Pow((Math.Cos(a1)), 2)) / 3;
            double b1 = ScaleFactor * p6 * (a1 - p3 * a6 + p4 * a7 - p5 * a8);
            double b2 = (northing - b1) / a2;
            double b3 = ((p2 * Math.Pow(a3, 2)) / 2) * Math.Pow((Math.Cos(a1)), 2);
            double b4 = a3 * (1 - (b3 / 3));
            double b5 = (b2 * (1 - b3)) + a1;
            double b6 = (Math.Exp(b4) - Math.Exp(-b4)) / 2;
            double b7 = Math.Atan(b6 / (Math.Cos(b5)));
            double b8 = Math.Atan(Math.Cos(b7) * Math.Tan(b5));

            double longitude = b7 * Rad2Deg + centralMeridian;
            double latitude = ((a1 + (1 + p2 * Math.Pow(Math.Cos(a1), 2) - (3.0 / 2.0) * p2 * Math.Sin(a1) * Math.Cos(a1) * (b8 - a1)) * (b8 - a1))) * Rad2Deg;

            return new GISVertex(longitude, latitude);
        }


        static double Deg2Rad = Math.PI / 180;
        static double v1 = 1 - SemiMinorAxis * SemiMinorAxis / (SemiMajorAxis * SemiMajorAxis);
        static double v2 = (v1) / (1 - v1);
        static double v3 = 1 - v1 / 4 - 3 * v1 * v1 / 64 - 5 * v1 * v1 * v1 / 256;
        static double v4 = 3 * v1 / 8 + 3 * v1 * v1 / 32 + 45 * v1 * v1 * v1 / 1024;
        static double v5= 15 * v1 * v1 / 256 + 45 * v1 * v1 * v1 / 1024;
        static double v6 = 35 * v1 * v1 * v1 / 3072;

        public static UTM_Vertex WGS2UTM(GISVertex v)
        {
            double Longitude = v.x;
            double Latitude = v.y;
            //限定经度在+-180之间
            ...
            return WGS2UTM(v, ZoneNumber, Latitude >= 0);
        }

        public static UTM_Vertex WGS2UTM(GISVertex v, int ZoneNumber, bool NorthOrSouth)
        {
            double Longitude = v.x;
            double Latitude = v.y;
            double OriginLongitude = ZoneNumber * 6 - 183;
            double a1 = Latitude * Deg2Rad;
            double a2 = Longitude * Deg2Rad;
            double a3 = OriginLongitude * Deg2Rad;
            double a4 = SemiMajorAxis / Math.Sqrt(1 - v1 * Math.Sin(a1) * Math.Sin(a1));
            double a5 = Math.Tan(a1) * Math.Tan(a1);
            double a6 = v2 * Math.Cos(a1) * Math.Cos(a1);
            double a7 = Math.Cos(a1) * (a2 - a3);
            double a8 = SemiMajorAxis * (v3 * a1 - v4 * Math.Sin(2 * a1) + v5 * Math.Sin(4 * a1) - v6 * Math.Sin(6 * a1));

            double X = ScaleFactor * a4 * (a7 + (1 - a5 + a6) * Math.Pow(a7, 3) / 6
            + (5 - 18 * a5 + a5 * a5 + 72 * a6 - 58 * v2) * Math.Pow(a7, 5) / 120) + FalseEasting;
            double Y = ScaleFactor * (a8 + a4 * Math.Tan(a1) * (a7 * a7 / 2 + (5 - a5 + 9 * a6 + 4 * a6 * a6) * Math.Pow(a7, 4) / 24
            + (61 - 58 * a5 + a5 * a5 + 600 * a6 - 330 * v2) * Math.Pow(a7, 6) / 720));
            //南半球偏移
            if (!NorthOrSouth) Y += SouthOffset;
            return new UTM_Vertex(X, Y, ZoneNumber, NorthOrSouth);
        }

        public static GISSpatial UTM2WGS(GISSpatial spatial, SHAPETYPE shapetype, int ZoneNumber, bool NorthOrSouth)
        {
            if (shapetype==SHAPETYPE.point)
            {
                GISPoint p = (GISPoint)spatial;
                GISVertex v = UTM2WGS(p.centroid, ZoneNumber, NorthOrSouth);
                return new GISPoint(v);
            }
            else if (shapetype == SHAPETYPE.line)
            {
                GISLine line = (GISLine)spatial;
                List<GISVertex> vs = new List<GISVertex>();
                for (int i = 0; i < line.Vertexes.Count; i++)
                    vs.Add(UTM2WGS(line.Vertexes[i], ZoneNumber, NorthOrSouth));
                return new GISLine(vs);
            }
            else if (shapetype == SHAPETYPE.polygon)
            {
                GISPolygon polygon = (GISPolygon)spatial;
                List<GISVertex> vs = new List<GISVertex>();
                for (int i = 0; i < polygon.Vertexes.Count; i++)
                    vs.Add(UTM2WGS(polygon.Vertexes[i], ZoneNumber, NorthOrSouth));
                return new GISPolygon(vs);
            }
            return null;
        }

        public static void UTM2WGS(GISVectorLayer layer)
        {
            if (layer.CSType != CSTYPE.utm) return;
            //转换每个空间对象
            for (int i = 0; i < layer.FeatureCount(); i++)
            {
                GISFeature f = layer.GetFeature(i);
                f.spatialpart = UTM2WGS(f.spatialpart, layer.ShapeType, layer.ZoneNumber, layer.NorthOrSouth);
            }
            //更新空间索引及空间范围
            if (layer.rtree != null) layer.BuildRTree();
            else layer.UpdateExtent();
            layer.CSType = CSTYPE.wgs84;
        }

        public static GISSpatial WGS2UTM(GISSpatial spatial, SHAPETYPE shapetype, int ZoneNumber, bool NorthOrSouth)
        {
            if (shapetype == SHAPETYPE.point)
            {
                GISPoint p = (GISPoint)spatial;
                GISVertex v = WGS2UTM(p.centroid, ZoneNumber,NorthOrSouth).v;
                return new GISPoint(v);
            }
            else if (shapetype == SHAPETYPE.line)
            {
                ...
                List<GISVertex> vs = new List<GISVertex>();
                for (int i = 0; i < line.Vertexes.Count; i++)
                    vs.Add(WGS2UTM(line.Vertexes[i], ZoneNumber, NorthOrSouth).v);
                return new GISLine(vs);
            }
            else if (shapetype == SHAPETYPE.polygon)
            {
                ...
                List<GISVertex> vs = new List<GISVertex>();
                for (int i = 0; i < polygon.Vertexes.Count; i++)
                    vs.Add(WGS2UTM(polygon.Vertexes[i], ZoneNumber, NorthOrSouth).v);
                return new GISPolygon(vs);
            }
            return null;
        }

        public static void WGS2UTM(GISVectorLayer layer, int ZoneNumber, bool NorthOrSouth)
        {
            if (layer.CSType != CSTYPE.wgs84) return;
            for (int i=0;i<layer.FeatureCount();i++)
            {
                GISFeature f = layer.GetFeature(i);
                f.spatialpart = WGS2UTM(f.spatialpart, layer.ShapeType, ZoneNumber, NorthOrSouth);                
            }
            //更新空间索引及空间范围
            if (layer.rtree != null) layer.BuildRTree();
            else layer.UpdateExtent();
            layer.CSType = CSTYPE.utm;
            layer.ZoneNumber = ZoneNumber;
            layer.NorthOrSouth = NorthOrSouth;
        }

        public static void WGS2UTM(GISVectorLayer layer)
        {
            if (layer.CSType != CSTYPE.wgs84) return;
            UTM_Vertex uv = WGS2UTM(layer.Extent.getCenter());
            WGS2UTM(layer, uv.ZoneNumber, uv.NorthOrSouth);
        }

        public static void UTM2WGS(GISRasterLayer layer)
        {
            if (layer.CSType != CSTYPE.utm) return;
            GISExtent extent = layer.Extent;
            GISVertex bottomleft = UTM2WGS(extent.bottomleft, layer.ZoneNumber, layer.NorthOrSouth);
            GISVertex upright = UTM2WGS(extent.upright, layer.ZoneNumber, layer.NorthOrSouth);
            layer.Extent = new GISExtent(bottomleft, upright);
            layer.CSType = CSTYPE.wgs84;
        }

        public static void WGS2UTM(GISRasterLayer layer, int ZoneNumber, bool NorthOrSouth)
        {
            if (layer.CSType != CSTYPE.wgs84) return;
            GISExtent extent = layer.Extent;
            GISVertex bottomleft = WGS2UTM(extent.bottomleft, ZoneNumber, NorthOrSouth).v;
            GISVertex upright = WGS2UTM(extent.upright, ZoneNumber,NorthOrSouth).v;
            layer.Extent = new GISExtent(bottomleft,upright);
            layer.CSType = CSTYPE.utm;
            layer.ZoneNumber = ZoneNumber;
            layer.NorthOrSouth = NorthOrSouth;
        ...
        public static void WGS2UTM(GISRasterLayer layer)
        {
            if (layer.CSType != CSTYPE.wgs84) return;
            UTM_Vertex uv = WGS2UTM(layer.Extent.getCenter());
            WGS2UTM(layer, uv.ZoneNumber, uv.NorthOrSouth);
    ...
    public class GISRasterLayer : GISLayer
    {
        ...
        public GISRasterLayer(string filename)
        {
            ...
            string[] values = objReader.ReadLine().Split(',');
            CSType = (CSTYPE)Enum.Parse(typeof(CSTYPE), values[0]);
            ZoneNumber = Int32.Parse(values[1]);
            NorthOrSouth = (values[2] == "1");
            //打开图片文件
    ...
    public class GISMyFile
    {
        ...
        struct MyFileHeader
        {
            ...
            public int CSType, ZoneNumber;
            public bool HasTree, NorthOrSouth;
        };
        ...
        static void WriteFileHeader(GISVectorLayer layer, BinaryWriter bw, bool OnlySelected)
        {
            ...
            mfh.CSType = (int)(layer.CSType);
            mfh.ZoneNumber = layer.ZoneNumber;
            mfh.NorthOrSouth = layer.NorthOrSouth;
        ...
        public static List<GISVectorLayer> ReadFileMultiLayers(string filename)
        {
            ...
            while (br.BaseStream.Position < br.BaseStream.Length)
            {
                ...
                CSTYPE CSType = (CSTYPE)Enum.Parse(typeof(CSTYPE), mfh.CSType.ToString());
                //先选择不建立索引，因为可以稍后增加
                GISVectorLayer layer = new GISVectorLayer(layername, ShapeType, Extent, Fields, false, 
                    CSType,mfh.ZoneNumber,mfh.NorthOrSouth);
    ...
    public class GISVectorLayer:GISLayer
    {
        ...
        public RTree rtree;
        public GISVectorLayer(string _name, SHAPETYPE _shapetype, GISExtent _extent, List<GISField> _fields = null,
            bool NeedIndex = true, CSTYPE cstype = CSTYPE.wgs84, int zonenumber = -1, bool northorsouth = false)
        {
    ...
    public class GISShapefile
    {
        ...
        public static GISVectorLayer ReadShapeFile(string shpfilename)
        {
            ...
            string prjfilename = shpfilename.Replace(".shp", ".prj");
            StreamReader objReader = new StreamReader(prjfilename);
            //读取prj的全部内容
            string wkt = objReader.ReadLine(); 
            int topos = wkt.IndexOf("\",");
            //坐标系统名称部分
            wkt = wkt.Substring(0, topos);
            CSTYPE CStype = CSTYPE.others;
            int ZoneNumber = -1;
            bool NorthOrSouth = false;
            //if (wkt.IndexOf("GEOGCS")>-1)  //如果把所有地理坐标系统都认为是WGS84
            if (wkt.IndexOf("GCS_WGS_1984")>-1)
                CStype = CSTYPE.wgs84;
            //else if (wkt.IndexOf("_UTM_Zone_") > -1) //如果把所有UTM都认为是基于WGS84的
            else if (wkt.IndexOf("WGS_1984_UTM_Zone_") > -1)
            {
                CStype = CSTYPE.utm;
                int frompos = wkt.IndexOf("Zone_");
                string zn = wkt.Substring(frompos + 5,wkt.Length-frompos - 5-1);
                ZoneNumber = Int32.Parse(zn);
                string ns = wkt.Substring(wkt.Length - 1);
                NorthOrSouth = (ns == "N");
            }            
            GISVectorLayer layer = new GISVectorLayer(GISTools.NamePart(shpfilename), ShapeType, extent, ReadFields(table),true,
                CStype,ZoneNumber,NorthOrSouth);
            int rowindex = 0;
